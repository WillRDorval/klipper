...
int __visible
stepcompress_had_position(struct stepcompress *scx, struct stepcompress *scy, int64_t x, int64_t y, int64_t err, uint64_t clock){
    int64_t px = scx->last_position;
    int64_t py = scy->last_position;
    struct history_steps *hsx = list_first_entry(&scx->history_list, struct history_steps, node);
    struct history_steps *hsy = list_first_entry(&scy->history_list, struct history_steps, node);
    uint64_t cutoff = (hsx->last_clock > hsy->last_clock ? hsy->last_clock : hsx->last_clock) - clock;
    while (hsx->last_clock >= cutoff && hsy->last_clock >= cutoff) {

        if ((hsx->node.next != &(scx->history_list.root))){
            if (hsx->last_clock < cutoff) {
                if (hsx->step_count > 0){
                    if (hsx->start_position - err > x || hsx->start_position+hsx->step_count + err < x) {
        ...
        if ((hsy->node.next != &(scy->history_list.root))) {
            if (hsy->last_clock < cutoff){
                if (hsy->step_count > 0){
                    if (hsy->start_position - err > y || hsy->start_position+hsy->step_count + err < y) {
                        hsy = list_next_entry(hsy, node);
                        continue;
                    }
                } else {
                    if (hsy->start_position + err < y || hsy->start_position+hsy->step_count - err > y) {
        ...
        if (hsy->first_clock > hsx->last_clock){
            hsy = list_next_entry(hsy, node);
            continue;
        }

        //both nodes are over the correct point in space and overlap in time

        int64_t cx_min, cx_max;

        if (hsx->add){
            cx_min = (double) (sqrt((double)(4*hsx->add*hsx->interval*(x-err + hsx->start_position)))-1.)/2.* (double)hsx->add;
            cx_max = (double) (sqrt((double)(4*hsx->add*hsx->interval*(x+err + hsx->start_position)))-1.)/2.* (double)hsx->add;
        } else {
            // x+- err start (x, t), slope
            cx_min = abs(hsx->interval*(x-err + hsx->start_position));
            cx_max = abs(hsx->interval*(x+err + hsx->start_position));
        }

        if (cx_max > cx_min){
            if (cx_max < hsy->first_clock){
                hsy = list_next_entry(hsy, node);
                continue;
            }
            if (cx_min > hsy->last_clock){
                hsx = list_next_entry(hsx, node);
                continue;
            }
        } else{
            if (cx_min < hsy->first_clock){
                hsy = list_next_entry(hsy, node);
                continue;
            }
            if (cx_max > hsy->last_clock){
                hsx = list_next_entry(hsx, node);
                continue;
            }
        }

        int64_t cy_min, cy_max;

        if (hsy->add){
            cy_min = (double) (sqrt((double)(4*hsy->add*hsy->interval*(y-err + hsy->start_position)))-1.)/2.* (double)hsy->add;
            cy_max = (double) (sqrt((double)(4*hsy->add*hsy->interval*(y+err + hsy->start_position)))-1.)/2.* (double)hsy->add;
        } else {
            // x+- err start (x, t), slope
            cy_min = abs(hsy->interval*(y-err + hsy->start_position));
            cy_max = abs(hsy->interval*(y+err + hsy->start_position));
        }
        ...
        if (cy_max > cy_min){
            if (cx_max > cx_min){
                ...
                return (MAX(cx_min, cy_min)+MIN(cx_max, cy_max))/2;
            } else {
                if (cy_max < cx_max){
                    hsx = list_next_entry(hsx, node);
                    continue;
